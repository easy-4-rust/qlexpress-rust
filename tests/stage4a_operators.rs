...
fn bin(op: &dyn BinaryOperator, l: DataValue, r: DataValue) -> DataValue {
    op.execute(&v(l), &v(r), &mut ctx(), &opts(), &PureErrReporter::INSTANCE).unwrap()
}

fn bin_err(op: &dyn BinaryOperator, l: DataValue, r: DataValue) -> qlexpress_rust::exception::QLException {
    op.execute(&v(l), &v(r), &mut ctx(), &opts(), &PureErrReporter::INSTANCE).unwrap_err()
}
...
fn plus_adds_numbers_and_concats_strings() {
    ...
    assert_eq!(bin(&op, DataValue::Int(1), DataValue::Int(2)), DataValue::Int(3));
    ...
    // int + long → long 提升。
    assert_eq!(bin(&op, DataValue::Int(1), DataValue::Long(1)), DataValue::Long(2));
    // 字符串拼接。
    assert_eq!(
...
fn minus_multiply_basic() {
    assert_eq!(bin(&MinusOperator::get_instance(), DataValue::Int(5), DataValue::Int(8)), DataValue::Int(-3));
    // 'a' - 1 = 96(字符按码点)。
    assert_eq!(bin(&MinusOperator::get_instance(), DataValue::Char('a'), DataValue::Int(1)), DataValue::Int(96));
    assert_eq!(bin(&MultiplyOperator::get_instance(), DataValue::Int(3), DataValue::Int(4)), DataValue::Int(12));
    // long 溢出回绕。
    assert_eq!(
        bin(&MultiplyOperator::get_instance(), DataValue::Long(i64::MAX), DataValue::Long(2)),
        DataValue::Long(-2)
    );
}
...
fn divide_semantics_aligned_with_java() {
    ...
    assert_eq!(bin(&op, DataValue::Int(7), DataValue::Int(2)), DataValue::BigDec("3.5".into()));
    // 非终止小数 → scale 10,HALF_UP。
    assert_eq!(bin(&op, DataValue::Int(1), DataValue::Int(3)), DataValue::BigDec("0.3333333333".into()));
    assert_eq!(bin(&op, DataValue::Int(2), DataValue::Int(3)), DataValue::BigDec("0.6666666667".into()));
    // 浮点除零 → Infinity(不 panic)。
    assert_eq!(bin(&op, DataValue::Double(1.0), DataValue::Int(0)), DataValue::Double(f64::INFINITY));
...
fn remainder_sign_and_float() {
    ...
    // 符号跟被除数(Java % 语义)。
    assert_eq!(bin(&op, DataValue::Int(-7), DataValue::Int(3)), DataValue::Int(-1));
    assert_eq!(bin(&op, DataValue::Long(7), DataValue::Int(-3)), DataValue::Long(1));
    // 浮点取余。
    assert_eq!(bin(&op, DataValue::Double(5.5), DataValue::Int(2)), DataValue::Double(1.5));
    // 未注册词素。
...
fn assign_operators_write_back() {
    ...
    let r = PlusAssignOperator::get_instance()
        .execute(&l, &v(DataValue::Int(2)), &mut ctx(), &opts(), &PureErrReporter::INSTANCE)
        .unwrap();
    ...
    let r = MinusAssignOperator::get_instance()
        .execute(&l, &v(DataValue::Int(4)), &mut ctx(), &opts(), &PureErrReporter::INSTANCE)
        .unwrap();
    ...
    RemainderAssignOperator::get_instance()
        .execute(&l, &v(DataValue::Int(3)), &mut ctx(), &opts(), &PureErrReporter::INSTANCE)
        .unwrap();
    assert_eq!(l.get(), DataValue::Int(1));

    // 非左值 → INVALID_ASSIGNMENT。
    let err = PlusAssignOperator::get_instance()
        .execute(&v(DataValue::Int(1)), &v(DataValue::Int(2)), &mut ctx(), &opts(), &PureErrReporter::INSTANCE)
        .unwrap_err();
...
fn bitwise_ops_on_numbers_and_booleans() {
    assert_eq!(bin(&BitwiseAndOperator::get_instance(), DataValue::Int(0b110), DataValue::Int(0b101)), DataValue::Int(0b100));
    assert_eq!(bin(&BitwiseOrOperator::get_instance(), DataValue::Int(0b110), DataValue::Long(0b101)), DataValue::Long(0b111));
    assert_eq!(bin(&BitwiseXorOperator::get_instance(), DataValue::Int(0b110), DataValue::Int(0b101)), DataValue::Int(0b011));
    // Boolean 操作数按逻辑位运算;null 视为 false。
    assert_eq!(bin(&BitwiseAndOperator::get_instance(), DataValue::Bool(true), DataValue::Null), DataValue::Bool(false));
    assert_eq!(bin(&BitwiseOrOperator::get_instance(), DataValue::Bool(false), DataValue::Bool(true)), DataValue::Bool(true));
    assert_eq!(bin(&BitwiseXorOperator::get_instance(), DataValue::Bool(true), DataValue::Bool(true)), DataValue::Bool(false));
}

#[test]
fn shift_mask_and_sign_extension() {
    // int 移位距离掩 31:1 << 32 == 1。
    assert_eq!(bin(&BitwiseLeftShiftOperator::get_instance(), DataValue::Int(1), DataValue::Int(32)), DataValue::Int(1));
    // 算术右移符号扩展:-8 >> 1 == -4。
    assert_eq!(bin(&BitwiseRightShiftOperator::get_instance(), DataValue::Int(-8), DataValue::Int(1)), DataValue::Int(-4));
    // 逻辑右移补零:-1 >>> 1 == 0x7FFFFFFF。
    assert_eq!(bin(&BitwiseRightShiftUnsignedOperator::get_instance(), DataValue::Int(-1), DataValue::Int(1)), DataValue::Int(0x7FFF_FFFF));
    // long 域:-1L >>> 1 == i64::MAX。
    assert_eq!(bin(&BitwiseRightShiftUnsignedOperator::get_instance(), DataValue::Long(-1), DataValue::Int(1)), DataValue::Long(i64::MAX));
    // 移位距离为浮点 → 报错(Java UnsupportedOperationException)。
    assert!(bin_err(&BitwiseLeftShiftOperator::get_instance(), DataValue::Int(1), DataValue::Double(1.0))
        .reason()
...
fn bitwise_assign_variants() {
    ...
    BitwiseAndAssignOperator::get_instance()
        .execute(&l, &v(DataValue::Int(0b101)), &mut ctx(), &opts(), &PureErrReporter::INSTANCE)
        .unwrap();
    ...
    BitwiseXorAssignOperator::get_instance()
        .execute(&l, &v(DataValue::Int(1)), &mut ctx(), &opts(), &PureErrReporter::INSTANCE)
        .unwrap();
...
fn bitwise_invert_unary() {
    ...
    assert_eq!(op.execute(&v(DataValue::Int(0)), &PureErrReporter::INSTANCE).unwrap(), DataValue::Int(-1));
...
fn equal_unequal_across_numeric_types() {
    ...
    assert_eq!(bin(&eq, DataValue::Int(1), DataValue::Long(1)), DataValue::Bool(true));
    assert_eq!(bin(&eq, DataValue::Long(1), DataValue::Double(1.0)), DataValue::Bool(true));
    assert_eq!(bin(&eq, DataValue::BigDec("1.00".into()), DataValue::Int(1)), DataValue::Bool(true));
    // 'a' == 97(字符按码点)。
    assert_eq!(bin(&eq, DataValue::Char('a'), DataValue::Int(97)), DataValue::Bool(true));
    assert_eq!(bin(&eq, DataValue::Null, DataValue::Null), DataValue::Bool(true));
    assert_eq!(bin(&eq, DataValue::Null, DataValue::Int(1)), DataValue::Bool(false));

    let ne = UnequalOperator::get_instance("!=").unwrap();
    assert_eq!(bin(&ne, DataValue::Int(1), DataValue::Double(1.5)), DataValue::Bool(true));
    let ne2 = UnequalOperator::get_instance("<>").unwrap();
    assert_eq!(bin(&ne2, DataValue::Int(1), DataValue::Long(1)), DataValue::Bool(false));
    assert!(UnequalOperator::get_instance("=?").is_none());
}

#[test]
fn ordering_compares_and_avoid_null_pointer() {
    assert_eq!(bin(&GreaterOperator::get_instance(), DataValue::Int(2), DataValue::Double(1.5)), DataValue::Bool(true));
    assert_eq!(bin(&GreaterEqualOperator::get_instance(), DataValue::Int(1), DataValue::Long(1)), DataValue::Bool(true));
    assert_eq!(bin(&LessOperator::get_instance(), DataValue::Str("a".into()), DataValue::Str("b".into())), DataValue::Bool(true));
    assert_eq!(bin(&LessEqualOperator::get_instance(), DataValue::Int(2), DataValue::Int(1)), DataValue::Bool(false));
    ...
    let r = GreaterOperator::get_instance()
        .execute(&v(DataValue::Null), &v(DataValue::Int(1)), &mut ctx(), &opts, &PureErrReporter::INSTANCE)
        .unwrap();
    assert_eq!(r, DataValue::Bool(false));
    // 默认模式:null 比较报 INVALID_BINARY_OPERAND。
    assert_eq!(
        bin_err(&GreaterOperator::get_instance(), DataValue::Null, DataValue::Int(1)).error_code(),
        error_codes::INVALID_BINARY_OPERAND
    );
}
...
fn logic_and_or_truth_rules() {
    ...
    assert_eq!(bin(&and, DataValue::Bool(true), DataValue::Bool(true)), DataValue::Bool(true));
    assert_eq!(bin(&and, DataValue::Bool(true), DataValue::Null), DataValue::Bool(false));
    assert_eq!(bin(&and_word, DataValue::Null, DataValue::Null), DataValue::Bool(false));
    ...
    assert_eq!(bin(&or, DataValue::Bool(false), DataValue::Null), DataValue::Bool(false));
    assert_eq!(bin(&or_word, DataValue::Null, DataValue::Bool(true)), DataValue::Bool(true));
...
fn logic_not_unary() {
    ...
    assert_eq!(op.execute(&v(DataValue::Bool(false)), &PureErrReporter::INSTANCE).unwrap(), DataValue::Bool(true));
    // !null == true。
    assert_eq!(op.execute(&v(DataValue::Null), &PureErrReporter::INSTANCE).unwrap(), DataValue::Bool(true));
}
...
fn instanceof_operator() {
    ...
    assert_eq!(bin(&op, DataValue::Int(1), int_class.clone()), DataValue::Bool(true));
