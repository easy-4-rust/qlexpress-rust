//! 逻辑与操作符 `&&` / `and`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.logic.LogicAndOperator
//! (@author bingo)。
...
/// 逻辑与操作符。
///
/// 对应 Java: LogicAndOperator(带词素参数的实例缓存 `预注册 "&&" 与 "and"`;
...
impl LogicAndOperator {
    ...
    pub fn get_instance(operator: &'static str) -> Option<Self> {
        match operator {
            ...
            "and" => Some(LogicAndOperator { operator: "and" }),
            _ => None,
        }
    }
}
...
impl BinaryOperator for LogicAndOperator {
    /// 对应 Java 方法: `execute(...)` —— 逻辑与(null 视为 false,非 Boolean 报 INVALID_BINARY_OPERAND)。
    fn execute(
        &self,
        left: &QValue,
        right: &QValue,
    ...
    /// 对应 Java 方法: `getPriority()` —— QLPrecedences.AND。
