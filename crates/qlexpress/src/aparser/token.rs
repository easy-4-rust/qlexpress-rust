...
/// `Token` 结构体的 Rust 实现，保留对应对象的领域职责与公开契约。
...
impl Token {
    ...
    /// 处理 char position in line 对应的领域职责。
    /// 无显式参数；返回：`i32`。
