//! 取余赋值操作符 `%=`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.arithmetic.RemainderAssignOperator
//! (@author bingo)。
...
/// 取余赋值操作符。
///
/// 对应 Java: RemainderAssignOperator(复合赋值操作符:要求左操作数为 LeftValue,
/// 计算后写回并返回结果)。
pub struct RemainderAssignOperator;
...
impl BinaryOperator for RemainderAssignOperator {
    /// 对应 Java 方法: `execute(...)`。
    ///
    /// 语义要点(Java 原版逐行对齐):先 `assertLeftValue`,再计算
    /// `remainder(...)`,随后 `leftValue.set(result, errorReporter)` 写回
    ...
    fn execute(
        ...
        let result = BaseBinaryOperator::remainder("%=", left, right, ql_options, error_reporter)?;
