//! 加赋值操作符 `+=`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.arithmetic.PlusAssignOperator
//! (@author bingo)。
...
/// 加赋值操作符。
///
/// 对应 Java: PlusAssignOperator(复合赋值操作符:要求左操作数为 LeftValue,
/// 计算后写回并返回结果)。
pub struct PlusAssignOperator;

impl PlusAssignOperator {
    /// Java `getInstance()` 单例获取(无状态,直接构造)。
    /// 对应 Java：`PlusAssignOperator#getInstance()`。
    pub fn get_instance() -> Self {
        PlusAssignOperator
    }
}

impl BinaryOperator for PlusAssignOperator {
    /// 对应 Java 方法: `execute(...)`。
    ///
    /// 语义要点(Java 原版逐行对齐):先 `assertLeftValue`,再计算
    /// `plus(...)`,随后 `leftValue.set(result, errorReporter)` 写回
    ...
    fn execute(
        ...
        let result = BaseBinaryOperator::plus("+=", left, right, ql_options, error_reporter)?;
