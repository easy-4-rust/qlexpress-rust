//! 除赋值操作符 `/=`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.arithmetic.DivideAssignOperator
//! (@author bingo)。
...
/// 除赋值操作符。
///
/// 对应 Java: DivideAssignOperator(复合赋值操作符:要求左操作数为 LeftValue,
/// 计算后写回并返回结果)。
pub struct DivideAssignOperator;

impl DivideAssignOperator {
    /// Java `getInstance()` 单例获取(无状态,直接构造)。
    /// 对应 Java：`DivideAssignOperator#getInstance()`。
    pub fn get_instance() -> Self {
        DivideAssignOperator
    }
}

impl BinaryOperator for DivideAssignOperator {
    /// 对应 Java 方法: `execute(...)`。
    ///
    /// 语义要点(Java 原版逐行对齐):先 `assertLeftValue`,再计算
    /// `divide(...)`,随后 `leftValue.set(result, errorReporter)` 写回
