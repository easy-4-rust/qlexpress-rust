//! 位运算复合赋值操作符 `^=`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.bit.BitwiseXorAssignOperator
//! (@author bingo)。
...
/// 位运算复合赋值操作符 `^=`。
///
/// 对应 Java: BitwiseXorAssignOperator(复合赋值操作符:要求左操作数为 LeftValue,
/// 计算后写回并返回结果)。
pub struct BitwiseXorAssignOperator;
...
impl BinaryOperator for BitwiseXorAssignOperator {
    /// 对应 Java 方法: `execute(...)`。
    ///
    /// 语义要点(Java 原版逐行对齐):先 `assertLeftValue`,再计算
    /// `bitwise_xor(...)`,随后 `leftValue.set(result, errorReporter)` 写回
    ...
    fn execute(
        ...
        let result = BaseBinaryOperator::bitwise_xor("^=", left, right, error_reporter)?;
