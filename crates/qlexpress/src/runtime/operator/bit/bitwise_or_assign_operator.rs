//! 位运算复合赋值操作符 `|=`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.bit.BitwiseOrAssignOperator
//! (@author bingo)。
...
/// 位运算复合赋值操作符 `|=`。
///
/// 对应 Java: BitwiseOrAssignOperator(复合赋值操作符:要求左操作数为 LeftValue,
/// 计算后写回并返回结果)。
pub struct BitwiseOrAssignOperator;
...
impl BinaryOperator for BitwiseOrAssignOperator {
    /// 对应 Java 方法: `execute(...)`。
    ///
    /// 语义要点(Java 原版逐行对齐):先 `assertLeftValue`,再计算
    /// `bitwise_or(...)`,随后 `leftValue.set(result, errorReporter)` 写回
    ...
    fn execute(
        ...
        let result = BaseBinaryOperator::bitwise_or("|=", left, right, error_reporter)?;
