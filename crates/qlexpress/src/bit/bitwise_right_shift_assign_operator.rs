//! 位运算复合赋值操作符 `>>=`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.bit.BitwiseRightShiftAssignOperator
//! (@author bingo)。
...
/// 位运算复合赋值操作符 `>>=`。
///
/// 对应 Java: BitwiseRightShiftAssignOperator(复合赋值操作符:要求左操作数为 LeftValue,
/// 计算后写回并返回结果)。
pub struct BitwiseRightShiftAssignOperator;

impl BitwiseRightShiftAssignOperator {
    /// Java `getInstance()` 单例获取(无状态,直接构造)。
    /// 对应 Java：`BitwiseRightShiftAssignOperator#getInstance()`。
    pub fn get_instance() -> Self {
        BitwiseRightShiftAssignOperator
    }
}

impl BinaryOperator for BitwiseRightShiftAssignOperator {
    /// 对应 Java 方法: `execute(...)`。
    ///
    /// 语义要点(Java 原版逐行对齐):先 `assertLeftValue`,再计算
    /// `right_shift(...)`,随后 `leftValue.set(result, errorReporter)` 写回
    ...
    fn execute(
        ...
        let result = BaseBinaryOperator::right_shift(">>=", left, right, error_reporter)?;
