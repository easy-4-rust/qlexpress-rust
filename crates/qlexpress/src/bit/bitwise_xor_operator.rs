//! 按位异或操作符 `^`(Boolean 操作数按逻辑异或,null 视为 false)
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.bit.BitwiseXorOperator
//! (@author bingo)。
...
/// 按位异或操作符 `^`(Boolean 操作数按逻辑异或,null 视为 false)。
///
/// 对应 Java: BitwiseXorOperator(单例模式,`getInstance()`;Rust 为零大小类型)。
...
impl BinaryOperator for BitwiseXorOperator {
    ...
    fn execute(
        ...
        BaseBinaryOperator::bitwise_xor("^", left, right, error_reporter)
    }
    ...
    /// 对应 Java 方法: `getPriority()` —— QLPrecedences.XOR。
