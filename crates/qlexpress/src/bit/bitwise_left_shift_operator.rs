//! 左移操作符 `<<`(距离按位宽掩码)
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.bit.BitwiseLeftShiftOperator
//! (@author bingo)。
...
/// 左移操作符 `<<`(距离按位宽掩码)。
///
/// 对应 Java: BitwiseLeftShiftOperator(单例模式,`getInstance()`;Rust 为零大小类型)。
pub struct BitwiseLeftShiftOperator;

impl BitwiseLeftShiftOperator {
    /// Java `getInstance()` 单例获取(无状态,直接构造)。
    /// 对应 Java：`BitwiseLeftShiftOperator#getInstance()`。
    pub fn get_instance() -> Self {
        BitwiseLeftShiftOperator
    }
}

impl BinaryOperator for BitwiseLeftShiftOperator {
    ...
    fn execute(
        ...
        BaseBinaryOperator::left_shift("<<", left, right, error_reporter)
    }
