//! 大于等于操作符 `>=`
//!
//! 对应 Java: com.alibaba.qlexpress4.runtime.operator.compare.GreaterEqualOperator
//! (@author bingo)。
...
/// 大于等于操作符 `>=`。
///
/// 对应 Java: GreaterEqualOperator(单例模式,`getInstance()`;Rust 为零大小类型)。
pub struct GreaterEqualOperator;

impl GreaterEqualOperator {
    /// Java `getInstance()` 单例获取(无状态,直接构造)。
    /// 对应 Java：`GreaterEqualOperator#getInstance()`。
    pub fn get_instance() -> Self {
        GreaterEqualOperator
    }
}

impl BinaryOperator for GreaterEqualOperator {
