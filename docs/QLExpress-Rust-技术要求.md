...
| 口径 | 覆盖/总数 | 覆盖率 | 结论 |
|---|---:|---:|---|
| `crates/qlexpress/src/**` 核心行 | 26,707 / 30,257 | **88.27%** | Rust 回归覆盖观察值；不与 Java 语义等价 |
| `crates/qlexpress/src/**` 核心分支 | 2,817 / 3,734 | **75.44%** | nightly 显式分支口径 |
| workspace 行 | 27,221 / 32,674 | **83.31%** | 单独披露，不作为核心对标；差分执行器扩展增加未由 cargo test 调用的命令路径 |
| workspace 分支 | 2,859 / 3,808 | **75.08%** | nightly 显式分支口径 |
| workspace function | 3,155 / 3,733 | **84.52%** | 持续提升 |
| workspace region | 41,024 / 50,015 | **82.02%** | 持续提升 |
