...
- 方法 **199 IMPLEMENTED + 3 PLATFORM_NA + 1,612 UNVERIFIED；RUST_EXTENSION 13 → 0**（审计报告 `verification/RUST_EXTENSION-audit.md` 的 13 条全部以 reviewed_disposition 升级）
...
1,612 个 UNVERIFIED 是现行严格证据口径的真实账面（实现存在、差分/回放通过，但缺逐方法显式测试标记），后续按"高频方法优先"逐批补 disposition 升级。
