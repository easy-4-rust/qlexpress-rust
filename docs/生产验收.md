...
| 验收域 | 实际结果 |
|---|---|
| Java/Rust 自动差分 | `differential.jsonl` 284/284 一致，0 mismatch / 0 missing；其中 41 项直接调用类型化 `NumberMath` 门面，74 项逐方法调用五个具体数值域，41 项覆盖 `BaseBinaryOperator`，44 项直接覆盖 `OperatorManager` 注册、替换、查找、别名、优先级及适配器执行，2 项直接覆盖 `DelegateQContext` 的 19 个构造/公开方法及 `QScope` 的 12 个接口方法、状态副作用和全局关闭异常，1 项直接覆盖 `FixedSizeStack`/`Parameters` 的 10 个方法及 Java 原数组窗口覆盖副作用，1 项直接覆盖 `QRuntime`/`QvmRuntime`/`QContext` 的 11 个方法、对象身份和附件双向写穿，1 项直接覆盖 `ExceptionTable` 的 3 个构造/公开方法、声明顺序、父类 catch 和 finally；统一比较结果类型、值、身份关系、NaN/Infinity、错误码与错误原因 |
| 已知直接差分 | `operator-manager-java-contract-divergences.jsonl` 0/1：未知词素 `precedence` 在 Java 实现中抛 NPE，Rust 按接口契约返回 `None`；明确计为 `PARTIAL`，不并入绿色用例 |
...
| nightly 辅助覆盖率复核 | workspace 行 27,197/32,650=83.30%、函数 3,155/3,733=84.52%、分支 2,859/3,808=75.08%；核心行 26,683/30,233=88.26%、核心函数 3,099/3,508=88.34%、核心分支 2,817/3,734=75.44%；全量测试通过；只用于定位未触达 Rust 分支，不证明、也不参与跨语言功能一致性判定 |
