...
- 方法 **1,811 IMPLEMENTED + 3 PLATFORM_NA；UNVERIFIED/RUST_EXTENSION/MISSING/STUB/PARTIAL 全部 0**（disposition 1,811/1,811 全匹配，锚点逐条经校验器对当前源码树实测）
- Java 测试映射 223/223 MAPPED；rust_tests 1,418
- 顺带修复：dispositions 文件 296 条失效锚点（旧测试符号/漂移路径/模板符号/旧级别词表）→ 校验 0 失败；删除 2 组重复键；`rust_source_fingerprint` 刷新至当前源码树；QLexer::tokenize 恢复 IMPLEMENTED（改用词法单测 + 18 例 lexer 现场差分金标准锚点）
