#!/usr/bin/env python3
"""Build the pinned Java baseline and compare Java/Rust QLexer token streams."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--corpus",
        type=Path,
        default=Path("verification/corpus/lexer-differential.jsonl"),
    )
    ...
    java_output = output / "java-lexer-differential.jsonl"
    rust_output = output / "rust-lexer-differential.jsonl"
    ...
    classpath = os.pathsep.join(
        [
            str(java_project / "target/classes"),
            (java_project / "target/classpath.txt").read_text().strip(),
        ]
    )
    run(
        [
            ...
            "-cp",
            classpath,
            "com.easy4rust.qlexpress.JavaLexerDifferentialRunner",
            str(corpus),
            str(java_output),
        ],
        repository,
    ...
    mismatches = [
        {"id": case_id, "java": java_records[case_id], "rust": rust_records[case_id]}
    ...
    report_path = output / "lexer-differential-report.json"
    report_path.write_text(
        json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    print(json.dumps(report, ensure_ascii=False))
    if missing or mismatches:
        print(f"lexer differential report: {report_path}", file=sys.stderr)
        return 1
    return 0
